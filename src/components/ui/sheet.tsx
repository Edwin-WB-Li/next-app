  ...
  return (
    <SheetPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={`bg-card data-[state=open]:animate-in data-[state=closed]:animate-out fixed z-50 gap-4 p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 ${sideStyles[side]} ${className} `}
        {...props}
      >
        {children}
...
function SheetTitle({
  ...
  return (
    <DialogPrimitive.Title
      ref={ref}
      className={`text-card-foreground text-lg font-semibold ${className}`}
      {...props}
    />
  );
}
