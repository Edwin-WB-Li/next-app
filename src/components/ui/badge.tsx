...
function Badge({ className = "", variant = "default", priority, ...props }: BadgeProps) {
  ...
  return (
    <div
      className={`focus:ring-ring inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold transition-colors focus:ring-2 focus:ring-offset-2 focus:outline-none ${appliedStyle} ${className} `}
      {...props}
    />
  );
}
